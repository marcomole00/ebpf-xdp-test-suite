...
#include "inspect_ip_header.skel.h"
...
int main(int argc, const char **argv) {
  struct inspect_ip_header_bpf *skel = NULL;
  int err;
  ...
  skel = inspect_ip_header_bpf__open();
  if (!skel) {
  ...
  if (inspect_ip_header_bpf__load(skel)) {
...
cleanup:
  cleanup_iface();
  inspect_ip_header_bpf__destroy(skel);
